...
byId('priceForm').addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget,a=assetBy(f.elements.assetId.value),price=parseDecimal(f.elements.price.value);if(!a||!Number.isFinite(price)||price<0)return;a.price=price;a.updatedAt=Date.now();await dbPut('assets',a);closeDialog('priceModal');await reload();toast(t('priceUpdated'))});byId('quickUpdateForm').addEventListener('submit',async e=>{e.preventDefault();const form=e.currentTarget,btn=form.querySelector('.sheet-primary');btn.disabled=true;try{for(const i of $$('[data-asset-price]',form)){const a=assetBy(i.dataset.assetPrice),v=parseDecimal(i.value);if(a&&Number.isFinite(v)&&v>=0){a.price=v;a.updatedAt=Date.now();await dbPut('assets',a)}}for(const i of $$('[data-position-qty]',form)){const p=state.positions.find(x=>x.id===i.dataset.positionQty),v=parseDecimal(i.value);if(p&&Number.isFinite(v)){p.quantity=v;p.updatedAt=Date.now();await dbPut('positions',p)}}await reload();closeDialog('quickUpdateModal');requestAnimationFrame(()=>toast(t('changesSaved')))}finally{btn.disabled=false}});window.addEventListener('resize',()=>{if(byId('historyView').classList.contains('active'))drawChart()},{passive:true})}
