...
async function reload(){
  ...
  state.accounts=normalizedAccounts;state.assets=normalizedAssets;state.snapshots=normalizedSnapshots;
  if(migrated){
...
function renderPositions(){
  ...
  summary.innerHTML=state.accounts.map(a=>`<div class="summary-pill"><span class="summary-icon ${iconLenClass(accountIcon(a))}" style="background:${accountColor(a)};color:#fff">${escapeHTML(accountIcon(a))}</span><span><small>${escapeHTML(a.name)}</small><strong>${visibleMoney(accountTotal(a.id))}</strong></span></div>`).join('');
  ...
  list.innerHTML=state.positions.slice().sort((a,b)=>positionValue(b)-positionValue(a)).map(p=>{const a=assetBy(p.assetId),acc=accountBy(p.accountId);return `<div class="list-card"><div class="list-icon ${iconLenClass(assetIcon(a))}" style="background:${assetColor(a)};color:#fff">${escapeHTML(assetIcon(a))}</div><div class="list-main"><strong>${escapeHTML(a?.name||'Удалённый актив')}</strong><small>${escapeHTML(a?.code||'—')} · ${escapeHTML(acc?.name||'Удалённый счёт')} · ${number(p.quantity)} ед.</small></div><div class="list-value"><strong>${visibleMoney(positionValue(p))}</strong><small>${a?visibleMoney(a.price):'—'} / ед.</small></div><button class="menu-button" data-position-menu="${p.id}" aria-label="Действия с позицией">···</button></div>`}).join('');
}
